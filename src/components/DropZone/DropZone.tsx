...
import {
  useDropzone, DropzoneProps, DropzoneOptions, DropzoneState,
} from 'react-dropzone'
...
export const DropZone: FC<IFormikUiDropZoneProps> = ({
  ...
  const {
    ...
    accept, onDrop, disabled, multiple, ...rest,
  })
    ...
    <>
      ...
      </section>
      {
        withClearButton && !disabled && (
          <button className="clear-button" type="button" onClick={handleReset}>{clearButtonText}</button>
        )
      }
    </>
  )
